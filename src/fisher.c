				   ...
				   float *fisher_nxn, int nufact, int nrule, int ntheta, int lmax, int npol)
{
	...
	printf("starting prefacto adn legendre\n");
	for (ptrdiff_t lidx = 0; lidx < nell; lidx++)
	{
		// for eq 30
...
#pragma omp critical
	...
	free(p_theta_ell);
	free(prefactor);
	printf("fisher_nxn_sp done\n");
}

void fisher_nxnxnxn_sp(const float *sqrt_icov_ell, const float *f_ell_i, const double *thetas,
					   const double *ct_weights, const long long *rule, const float *weights,
					   float *fisher_nxn, int nufact, int nrule, int ntheta, int lmax, int npol)
{
